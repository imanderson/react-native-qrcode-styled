...
export default function SVGPieces({
  ...
  if (renderCustomPieceItem) {
    ...
    return /*#__PURE__*/React.createElement(React.Fragment, null, svgPiecesMatrix);
  }
  ...
  for (let y = 0; y < bitMatrix.length; y++) {
    for (let x = 0; x < bitMatrix.length; x++) {
      ...
      if (bitMatrix[y]?.[x] === 1) {
        ...
        let PathComponent = /*#__PURE__*/React.createElement(Path, {
          scale: pieceScale,
        ...
        if (pieceLiquidRadius) {
          if (outerEyesOptions && !isLiquidPieceInEyes(x, y, bitMatrix.length) || !outerEyesOptions) {
            ...
            if (bitMatrix[y]?.[x - 1] === 1 && bitMatrix[y - 1]?.[x] === 1) {
              svgPiecesMatrix.push( /*#__PURE__*/React.createElement(Path, {
                key: `${x}_${y}_topLeft`,
                d: d
              }));
            }
